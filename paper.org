...
- Compared to the supercomputer runs:
  ...
  - a bit harder because (1) loopback vs. IB (2) RAM/Cache contention
    (3) smaller matrix
** Trace
- With and w/o precise model


| trace of the first iterations | Kernel simple | Kernel complex |
|-------------------------------+---------------+----------------|
| Network simple                | NOK           | OK             |
|-------------------------------+---------------+----------------|
| Network complex               | NOK           | OK             |

Network complex may not be an issus if HPL mostly relies on large
message sizes.
** Global performance
