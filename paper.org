...
Figure\ref{fig:hpl_scalability} compares for several process numbers
the impact of the matrix order on total makespan and memory. 
...
elements is $N^{2}$ and the number of steps of the
