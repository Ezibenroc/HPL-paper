...
Finely tuning MPI applications (number of process, granularity, collective
...
information on how the run are conducted on modern supercomputers. Section\ref{sec:relwork}
...
Gold 6130 CPU with 16 cores per CPU. We disabled hyperthreading
and fixed the frequency to \SI{3.7}{\giga\hertz}. We used HPL version 2.2 compiled
...
\model{1}-\noise{0} for =dtrsm= and simply skipped every other
...
beginning of 2017 and built precise \modelp{2}\noise{1} models of
...
modified for Stampede. We refer to\cite{FIXME} for a detailed
...
More specifically, we explain how simulate on a single commodity node
scenarios at the scale of the TOP500 qualification runs. Although
simulation is generally longer than a real execution is requires significantly 
resource consuming. This emulation employs several non-trivial
