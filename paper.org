...
High-Performance Linpack is the benchmark used to rank supercomputers in the
TOP500 and for which a careful tuning of all parameters (problem size, grid
arrangement, granularity, collective operation algorithms, etc.) is
essential. With a power consumption of several MW per hour on a TOP500 machine,
test-running HPL on the whole machine for hours is extremely expensive. In this
article, we explain how we both extended the SimGrid's SMPI simulator and
slightly modified the open-source version of HPL to allow a fast emulation on a
single commodity computer at the scale of a supercomputer. We explain how to
model the different components (network, BLAS, ...) of the supercomputer to
obtain a faithful performance prediction and show that a careful modeling of
both spatial and temporal node variability is crucial. We validate our approach
by showing that the simulation predictions are systematically within a few
...
for TOP500 machines, makes it financially prohibitive to test-run HPL on the whole
...
but it can already be observed that this kernel model leads to a
...
take 8 bytes. Thankfully, the x86-64 architecture supports several
...
\begin{figure*}[!b]
  \begin{minipage}[t]{.6\linewidth}\null\vspace{-.75em}
    \subfigure[\texttt{dgemm} heterogeneity]{\includegraphics[width=.45\linewidth]{figures/kernels/dgemm_heterogeneity_calib.png}}%
    \subfigure[\texttt{dgemm} model]{\includegraphics[width=.45\linewidth]{figures/kernels/dgemm_model_calib.png}}
...
We took care of modeling the 5 more important BLAS kernels used in HPL
...
that a few nodes exhibit a slightly different behavior, which is
...
\TC{Attention: seulement dgemm a un modèle hétérogène, tous les autres kernels sont homogènes. Aussi, seulement dgemm a un modèle polynomial, les autres sont linéaires.}

\TC{Tel que c'est écrit là, on a l'impression qu'on a MH2 pour tous les BLAS et MH1 pour tous les kernels HPL, c'est faux...}

\TC{En fait, je trouve la dichotomie BLAS/HPL étrange, on traite *exactement* de la meme manière dtrsm et HPL\_dlatcpy par exemple. On devrait plutot faire une section dgemm, et une section "other kernels".}
** Clearpage                                                        :ignore:
...
#+RESULTS:
[[file:/tmp/babel-U7iI14/figureCt4iym.png]]
