...
- Noise option \noise{2} (heteroscedastic): The conditional variance
  of the residuals (\ie $\sigma^2$ given $M,N,K$) are modeled by polynomial
...
dgemm2 %>% 
    group_by(type) %>% 
    mutate(mnk= mnk + ifelse(type=="M2 (polynomial)",1.5E8,ifelse(type=="M3 (polynomial + noise)",3E8,0)),
           duration = ifelse(type %in% c("M2 (polynomial)","M3 (polynomial + noise)"),duration/1.05,duration)) %>% # This is because this prediction uses the HPL correction
    ungroup() -> dgemm2
dgemm2 %>% tail(n=10)
dgemm2 %>% str()
#+end_src
...
# A tibble: 10 x 11
 ...
 1 dgemm  1306  3172  2011     1     0     2 M3 (polyno~   0.605   8.63e9     25
 2 dgemm   259  1215  1047     1     6     2 M3 (polyno~   0.0252  6.29e8      1
 3 dgemm   640  1160  9441     1    24     2 M3 (polyno~   0.492   7.31e9     21
 4 dgemm   547  3908  3279     1    24     2 M3 (polyno~   0.487   7.31e9     21
 5 dgemm  1047  1215   259     1    10     2 M3 (polyno~   0.0226  6.29e8      1
 6 dgemm  1841  2133  1615     1    26     2 M3 (polyno~   0.469   6.64e9     19
 7 dgemm  1442   912  2781     1    12     2 M3 (polyno~   0.253   3.96e9     11
 8 dgemm  3136  9602   321     1     8     2 M3 (polyno~   0.629   9.97e9     29
 9 dgemm  4287   416  1304     1    20     2 M3 (polyno~   0.160   2.63e9      7
10 dgemm  1475   917  1719     1     8     2 M3 (polyno~   0.162   2.63e9      7
...
plot = plot + theme_bw() + ylab('Duration (s)') + 
        scale_x_continuous(name = 'M.N.K', breaks = (0:4)*3E9, limits=c(0,1.2E10)) 
plot = plot + scale_color_manual(values=MSet1, guide=F) + labs(color='(M2 and M3 are shifted to the right\n to improve readability)') + #    scale_color_brewer(palette="Set1")
    theme(legend.position = c(0.7,0.21), legend.title=element_text(size = 7, face = "italic")) +
...
#+RESULTS:
[[file:/tmp/babel-M9BfMg/figure4A1UiG.png]]
...
func$mode_name = factor(ifelse(func$mode=='reality', 'Reality', 'M1 (linear + noise)'))
...
#+RESULTS:
[[file:/tmp/babel-M9BfMg/figurezYIQjh.png]]
...
\begin{figure}[h]
    ...
    \caption{Comparison of HPL performance prediction with the reality}\vspace{-2em}
...
%  \includegraphics[width=\linewidth,page=1]{./figures/stampede_simgrid.pdf}
  ...
  \caption{Performance prediction of HPL on Stampede using SimGrid.}\vspace{-2em}
