...
as they underestimate the impact of the network congestion and of
system noise. Whenever actual
...
\caption{Typical runs of HPL}\vspace{-1.5em}
...
CPUs processors and Debian Stretch (Linux 4.9). 
...
handling: =dtrsm=, =dgemv=, =dswap=, =daxpy=,
...
overal performance, we handled them similarly to =dgemm= and
...
memory consumption decreased from about \NSI{13.5}{\giga\byte} to
...
allows to load several times =data= segment and to avoid costly
...
draw_reg <- function(df, breakpoints) {
    ...
    txt = data.frame(func=unique(df$func), msg_size=c(50, 50), duration=c(5e-5, 5e-5))
    plot = plot + geom_text(aes(label=func), color='black', data=txt, size=6)
    ...
    for(grp in unique(df$group)) {
        for(func in unique(df$func)) {
            index = df$group == grp & df$func == func
    ...
    plot = plot + facet_wrap('func', nrow=2) + theme(strip.background = element_blank(), strip.text.x = element_blank())
    return(plot)
}
...
draw_mpi_reg <- function(platform, breakpoints) {
    ...
    return(draw_reg(rbind(df_send, df_recv), breakpoints))
}
#+end_src
...
#+RESULTS:
[[file:/tmp/babel-M9BfMg/figurelsniX8.png]]
...
#+RESULTS:
[[file:/tmp/babel-M9BfMg/figurefAnk6i.png]]
