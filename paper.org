...
#+LATEX_HEADER:\tikzset{every picture/.style={/utils/exec={\sffamily}}}  % use sans-serif fonts for tikz figures
...
\begin{center}
    ...
    \begin{minipage}[t]{0.41\linewidth}\centering
        ...
        \scalebox{.95}{\begin{tabular}{ll|ll}
            && Reality & Simulation\\
            \hline
            \parbox[t]{2mm}{\multirow{4}{*}{\rotatebox[origin=c]{90}{Dahu}}} & \# nodes & \Num{32} & \Num{1}\\
             & Memory & \NSI{2}{\tera\byte} & \NSI{9}{\giga\byte}\\
             & Duration & \Num{1} hour & \Num{5} hours\\
             & Resources & \Num{32} node hours & \Num{1} node hours\\
             \hline
            \parbox[t]{2mm}{\multirow{4}{*}{\rotatebox[origin=c]{90}{Stampede}}} & \# nodes & \Num{6006} & \Num{1}\\
             & Memory & \NSI{120}{\tera\byte} & \NSI{19}{\giga\byte}\\
             & Duration & \Num{2} hours & \Num{62} hours\\
             & Resources & \Num{12000} node hours & \Num{62} node hours\\
        \end{tabular}}
    ...
    \begin{minipage}[t]{0.54\linewidth}
        ...
        \vspace{-.5em}
        \captionof{figure}{HPL performance on Dahu can be predicted within within a few percents of reality using a stochastic heterogeneous model for \texttt{dgemm} and a simple model for all other kernels and the nerwork}
...
operations, overlap, ...) is critical to obtain good performance on
...
parameters and therefore optimize their performance. This expenditure
of time, combined with the power consumption that often reaches several MW for TOP500
machines, makes it financially expensive to test-run
applications. Similar difficulties are encountered when (co-)designing
...
supercomputer with the SimGrid/SMPI\cite{simgrid,smpi} simulator. We
detail how we obtained faithful models for several key functions (\eg
=dgemm= and =dtrsm=) and managed to reduce the memory consumption from
more than a hundred terabytes to several gigabytes. This allowed us to
emulate on a single commodity server within 1-2 days a scenario
similar to the HPL run that was carried out on the 
Stampede cluster (hosted at TACC) in 2013 for the TOP500.
More importantly, we managed to
predict the performance of HPL on a recent cluster (running a thousand
MPI ranks) within a few percent of reality.
\CH{Why is the small cluster "more important"? I would start this sentence using "Moreover, ..". As a reader, I ask myself: The cluster was within a few percent, and the precision for the stampede run was ...?}

This article is organized as follows: Section\ref{sec:con} presents
the main characteristics of the HPL application and provides details on
...
simulation/) is the only possible approach when studying an application
...
all levels (\ie simulator, application, operating system) that were necessary to
...
the supercomputer (network, compute kernels) are modeled to
...
named Dahu that we could fully control. 
Table\ref{fig:typical_run} shows the parameters that were used for each of
these runs.
...
A first approach for estimating the performance of applications like HPL is
extrapolation \cite{hpl_prediction}. The main idea is twofold: (1) Using a
simple linear regression, one can run the application several times with small
and medium problem sizes to predict its makespan for larger sizes. (2) For a
given problem size, it is possible to have a good estimate of the application
performance by running only a subset of its iterations.  With this method, it is
possible to predict the performance of large HPL runs with an error of only a
few percents and a relatively low cost. However, this approach has several
flaws:
- It does not account for phenomena that could happen at scale (\eg a network collapse).
- The application is seen as a black box, the prediction only outputs a raw
  value, it is not possible to have a trace of the execution.
- To make any prediction, it is necessary to make real runs on the target
  platform. It is not possible to study what-if scenarios (\eg what if the
  network bandwidth was increased or the nodes heterogeneity was decreased?).
- The predictions are only possible with the same parameters. In particular, the
  number of rows and columns of the virtual grid: to predict the performance of
  a run using the whole supercomputer, it is required to perform several
  executions using all the nodes of the machine.

Performance prediction of MPI applications through simulation has been
...
its behavior at the time of the run. Even slight modifications 
(\eg to communication patterns) may make the trace inaccurate. It is
possible for simple applications (\eg =stencil=)
to extrapolate behavior from small-scale
...
\CH{seemed quite solid? Using another simulator was never up for debate - we are the developers of this simulator. I would delete this part and just say ... SimGrid, but our proposed approach is a priori also possible with SST}
...
heterogeneity\cite{Velho_TOMACS13}). If needed, communications that start or end
...
\NSI{32}{\giga\byte} of RAM, two 8-core Intel Xeon E5-2620 v4
CPUs processors and a Debian Stretch OS (Linux 4.9). 
...
\Num{30000} showed that $\approx \NSI{96}{\percent}$ of the time is spent in
...
%  \null\vspace{-1cm}
  ...
  \caption{Replacing the calls to computationally expensive functions by a model allows to emulate HPL at a larger scale.}\vspace{-1.5em}
\end{figure}
#+END_EXPORT
...
\begin{figure}[!t]
 ...
 {\begin{minipage}{1.0\linewidth}
  ...
  \end{minipage}}\vspace{-1.5em}
  \caption{Panel structure and allocation strategy.\label{fig:panel}}\vspace{-1.5em}
\end{figure}
#+END_EXPORT
...
%  \includegraphics[width=\linewidth,page=2]{./figures/scalability_plot_nbproc.pdf}
  \caption{Time complexity and memory consumption are linear in the number of processes but remain mildly quadratic with matrix rank.}\vspace{-1.5em}
...
* MODELING HPL KERNELS AND COMMUNICATIONS\hspace{-1cm}
# #+LaTeX: \def\@secfont{\sffamily\bfseries\section@raggedright\MakeTextUppercase}\makeatother
# #+LaTeX: \let\section=\osection
...
- Noise option \noise{0} (no noise): This is the simplest option. It consists in simply
...
\begin{figure*}[!t]
  ...
  \begin{minipage}[t]{\linewidth}\null\vspace{-.75em}
    ...
    \vspace{-1.5em}
    \caption{Illustrating the realism of modeling for BLAS and HPL functions.}\vspace{-1.5em}
    \label{fig:blas_var}
  \end{minipage}\hfill%
\end{figure*}
#+END_EXPORT
...
plot = plot + theme_bw() + ylab('Duration (s)') + 
    ...
    labs(color='CPU') 
ggsave(filename='figures/kernels/dgemm_heterogeneity_calib.png', plot=plot, width=3.9,height=4, dpi=200)
## ggsave(filename="figures/kernels/dgemm_heterogeneity.pdf", plot=plot, width=4,height=4)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-M9BfMg/figureyWAkEb.png]]
...
plot = plot + theme_bw() + ylab('Duration (s)') + 
        scale_x_continuous(name = 'M.N.K', breaks = (0:4)*3E9, limits=c(0,1.2E10)) 
plot = plot + scale_color_manual(values=MSet1, guide=F) + labs(color='(Both M2 models are shifted to\n the right to improve readability)') + #    scale_color_brewer(palette="Set1")
    theme(legend.position = c(1, 0), legend.justification=c(1, 0), legend.text=element_text(size=8), legend.box.background=element_rect(colour = "black"),
          panel.border=element_rect(colour = "black", fill=NA), legend.title=element_text(size = 7, face = "italic")) +
    geom_smooth(data=dgemm2[dgemm2$type=="Reality",], size=.2,
                color="black", method='lm', se=F, fullrange=T) +
    guides(colour = guide_legend(override.aes = list(alpha = 1)))
ggsave(filename="figures/kernels/dgemm_model_calib.png", plot=plot, width=3.9,height=4, dpi=200)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-sfW1Yb/figureTu01UX.png]]
...
ggsave(filename='figures/kernels/dlatcpy_model.png', plot=plot, width=3.9,height=4, dpi=200)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-l9yW3l/figureFdU9XL.png]]
...
think that faithfully simulating an HPL execution with such settings
...
- We simulated one rank per core to obtain a higher number
  ...
  one rank per node, as this (1) increases the amount of data
  transferred through MPI and (2) the performance is then subject to
  memory interference and network
  heterogeneity (which we modeled through different models for
...
- We used a smaller input matrix and the makespan is therefore
  reduced. This is generally harder to predict with a good precision.
...
ggsave(filename='figures/validation_performance_simple.pdf', height=2.2, width=6, plot=plot)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-11346F8S/figure11346ZyZ.png]]
...
\begin{figure}[h]
    % \centering
    \hbox{\hspace{-.05\linewidth}\resizebox{1.1\linewidth}{!}{\input{figures/validation_performance.tex}}\hspace{-.05\linewidth}}\vspace{-1.5em}
...
We have shown it is possible to predict consistently the
performance of HPL within a few percent of reality provided a
faithful model of each individual resource is used. We would like to stress
again that modeling both spatial and temporal variability is crucial.
...
%  \includegraphics[width=\linewidth,page=1]{./figures/stampede_simgrid.pdf}
  \includegraphics[height=4.8cm,page=1]{./figures/stampede_simgrid.pdf}\vspace{-1.5em}
  \caption{HPL on Stampede: predictions vs. reality}\vspace{-1.5em}
