...
get near to the peak performance, a very large input is needed and it
\CH{very large vs. sizeable}
...
\CH{Optimizations need to be optimized? - 2x optimization}
...
estimate the performance using simple mathematical performance
\CH{2x performance; isn't it ... by using?}
...
\CH{relevant = possible? Possible has a stronger emphasis.}
...
\CH{may or may not? I think we should say: It can!}
...
addresses, the page table rapidly becomes too large to for a single
...
complexity. The consequence of using simpler modeling options is
illustrated on HPL as a whole in Section\ref{sec:validation}.
\CH{I am not sure what this last sentence is supposed to mean. on hpl? with hpl? what does "hpl as a whole" mean? What consequence - lacking accuracy? We finally show in Section... that reducing the complexity of the models by removing parameters leads to a serious inaccuracy of the model.}
** Modeling Notations
...
\begin{figure}[!t]
  ...
  \vspace{-2em}
  \caption{Illustrating piecewise linearity and temporal variability of high-speed communications on two systems.}\vspace{-2em}
  \label{fig:nw_var}
\end{figure}
#+END_EXPORT
...
\CH{Maybe: used effortlessly directly ... ? avoid 2x with}
...
\CH{Over the whole node = on all nodes? What conditions are reproduced?}
...
\CH{Not sure what this means. A duration cannot be slow, it can be long?}
happens for some particular (e.g., tall and skinny) matrix geometries,
which calls using the full polynomial
\CH{which calls - not sure what this means here.}
...
infrequently called compared to =dgemm=. Finally, since variability clearly
increases with input parameters, a \noise{2} model is clearly
\CH{increases with number of input parameters? or the value of the input parameters? Also: Avoid clearly - it's a subjective word.}
...
- We simulated one rank per core to obtain a higher number
  ...
  transfered through MPI and (2) the performance is then subject to
...
- We used a smaller matrix, hence a lower simulation time, which is
...
and focus on the \si{\giga\flops} rate reported by HPL when varying
matrix size (see Figure\ref{fig:validation_performance}).
\CH{when varying matrix size - HPL reports this always, not just in this case. Not sure what this could mean.}
...
extremly close to reality as it slightly underestimates the performance by
...
certain that that only one MPI rank per node was used. For this
reason, it is likely that the KNC was used as an accelerator, which is
effortless with Intel's Math Kernel Library (MKL) as it supports
automatic offloading for selected BLAS functions. While we cannot
...
\CH{iterative nature of an iteration? Phase-based nature?}
