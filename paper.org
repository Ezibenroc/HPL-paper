...
environments. Since then, SimGrid has also been employed to study cloud and
peer-to-peer systems. SMPI\cite{smpi} is a recently developed
simulator based on the SimGrid API that supports faithful simulation
of unmodified MPI applications written in C/C++ or Fortan.  
...
peer-to-peer systems comprising two million nodes using only a
single standard machine, the main focus of the SimGrid development
#+LATEX: \AL{Add a citation for the paper with two million nodes}
...
Stampede1 run of HPL was done several years ago and we only have
very limited information about the setup (\eg software versions, background
processes, ...).
...
Transmission protocols (\ie short, eager, rendez-vous) can be selected
based on the message size, with each protocol incurring its own
synchronization semantics.
#+LATEX: \AL{Is \ie correct or should I rather use \eg for that list?}
...
#+LATEX: \AL{These discrepancies were not yet discussed here. And what is the challenge?}

** Application Behavior Modeling
We briefly explained in the previous section that SMPI relies on the /online/
...
reduce both memory footprint and simulation duration of an emulation run.
