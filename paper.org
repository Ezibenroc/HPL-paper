...
\begin{figure*}[!t]
  ...
  \begin{minipage}[t]{\linewidth}\null\vspace{-.75em}
    ...
    \vspace{-1em}
    \caption{Illustrating the realism of modeling for BLAS and HPL functions.}
    \label{fig:blas_var}
  \end{minipage}\hfill%
\end{figure*}
#+END_EXPORT
...
\begin{figure}[!b]
  \centering
  \includegraphics[width=.9\linewidth,page=1]{./figures/fullscale_unzoomed.png}
...
approach as much as possible using a full and unified Bayesian
estimation could be made with frameworks like STAN\cite{stan}. The
MCMC sampling technique used in STAN would also allow to easily inject
uncertainty (in particular the platform heterogeneity which should be
estimated from only a few sample nodes) in the simulation. This
integration effort is thus underway.

As a future work, building on the effort of SimGrid developers on
...
In this context, we believe that a faithful and
public reporting of the experimental conditions (compiler options,
library versions, HPL output, etc.) would be invaluable and allows
researchers to better understand how these platforms actually behave.
