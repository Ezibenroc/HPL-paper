...
and end timestamps of each kernel and MPI call. We limit the
...
translates in late senders/receivers that destroy the efficiency
...
line (a), on the top, is our first tentative to simulate HPL. The
...
network model, \noisep{1} (the line (b)), which unfortunately does
...
simulation as the performance is now overestimated by only 9% (the line (c) in the plot).
...
extremly close to the reality as it slightly underestimates the performance by
...
faithful model of each individual resource is used. Again, we
...
Alas, even a congestion-free network model only
...
broadcast.  Although the output states that the =long-modified= algorithm was
...
already draw two conclusions from our analysis: (1) the parameters by
HPL are not the ones used in the real
...
to aid compute centers with the decision of whether a new machine (and
what technology) will work best for a given application or if an
...
The downside of scaling that high is a less well-controlled scenario.
We explain how to model the performance of compute kernels and of
the MPI library and show that a careful modeling allows to predict
...
re-execution on the whole machine was impossible for us. We
...
efforts, the predicted performance is much lower than what was
...
was used in 2013 and misreports the true configuration. A quick
analysis of the optimized HPL binary confirmed that algorithmic
differences were likely to be the reason for the performance
differences.  We claim that the modifications we made in HPL are minor and
are applicable to that optimized version. In fact, while HPL comprises
...
underway.
As an other future work, building on the effort of SimGrid developers on
...
real applications (\eg BigDFT\cite{bigdft}) with other TOP500 machines.
...
library versions, input parameters, HPL output, etc.) would be invaluable and allow
researchers to better understand how to correctly model these platforms.
