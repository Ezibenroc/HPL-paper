...
supercomputer with the SimGrid/SMPI simulator. We
...
more than a hundred terabytes to several gigabytes. This allows us to
emulate in details scenarios similar to the HPL run conducted on the
Stampede cluster (TACC) in 2013 for the TOP500 on a single commodity
server in 1-2 days. More importantly, we manage to
predict the performance of HPL on a recent cluster (with a thousand of
MPI ranks) within a few percents of reality.
...
keep a large-scale run tractable. The scalability of our approach is
evaluated in Section\ref{sec:scalabilityevol}. In
Section\ref{sec:modeling}, we explain how the different resources of
the supercomputer (network, compute kernels) be modeled to
reflect faithfully the performance of HPL. In particular, we
