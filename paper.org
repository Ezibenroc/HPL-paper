...
\author{
    ...
    \IEEEauthorblockA{Univ. Grenoble Alpes, CNRS, Inria, Grenoble INP, LIG, 38000 Grenoble, France\\
      ...
      \includegraphics[width=.3\linewidth]{figures/node_hours.pdf} %
      \includegraphics[width=.6\linewidth]{figures/validation_performance_simple.pdf}
    }
}
...
# Labels definitions
lab = df %>% filter(
    (origin == 'Simulation' & matrix_size == 100000) |
...
max_x = max(dfsim$matrix_size)
max_y = max(dfsim$gflops)
lab = lab %>% mutate(x  = case_when(origin == 'Simulation' ~ 0.7*x,
                                    origin == 'Reality'    ~ 1.2*x
                                   ))
lab = lab %>% mutate(y  = case_when(origin == 'Simulation' ~ 1.05*y,
...
#+RESULTS:
[[file:/tmp/babel-fPmClk/figure0cThGd.png]]
