...
As explained in Section\ref{sec:em}, most of HPL computation
...
performance of parallel applications like HPL. Then, we consider
...
assuming \noise{1} so we implemented a method based on model trees
\cite[Chapter~9]{hastie_book}\cite{malerba} in a python library (=pytree=). When the noise
...
on Figure\ref{fig:nw_var}, the variability of high speed networks is
quite particular. Therefore, we took care of correctly estimating all the parameters for
...
** Modeling =dgemm=
=dgemm= is the compute kernel in which HPL spends most of the
