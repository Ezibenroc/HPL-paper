...
case for HPL. Furthermore, the size of the traces on Stampede
are with \SI{100}{\tera\byte} prohibitive.
#+LATEX: \TC{Also, the trace is simply too large? E.g. the trace for the Stampede's simulation would take more than 100GB (compressed).}
#+LATEX: \CH{@TOM: OK, TB? GB? Why should we mention SMPI traces?}
#+LATEX: \TC{@CH: Nope, GB, and this is a crude extrapolation. No idea about the state of the art trace formats.}
...
\CH{What do we do with this?}
\TC{For the large scale runs, no we don't do this, since the memory consumption is still an issue.}
...
#+LATEX: \CH{I think this is too detailed. Who cares if we pass MAP_HUGETLB?}
...
\SI{0.488}{\gibi\byte}. Secondly, simulation time decreases significantly since using hugepages
reduces the amount of page faults and the time spent by the kernel on page table
management. For a matrix of order $300,000$ and $64$ MPI
...
#+LATEX: \CH{I dislike that we give two problem sizes in one paragraph.}
# CH: Because I/O is now reduced.
...
#+LATEX: \AL{We need to explain why we believe this to be incorrect. See the output of that option in testing/ptest/HPL_pdinfo.c lines 1002, 1020, 1038}
...
#+LATEX: \AL{"Proc. of the ????" in the last item!}
#+LATEX: \TC{According to Google Scholar (https://scholar.google.com/citations?user=LeUoKXYAAAAJ), this is only a technical report, not peer-reviewed.}
