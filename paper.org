...
\TC{TODO}
- Kernels have several parameters but for each of them a very simple
  model can be identified (size)
  - Depict behavior for a few ones + MKL
  - Depict behavior of send receive for a few modern platforms
- Most kernels are mostly explained by one parameter but have
  different and significant variability (heteroskedastic + asymetric
  noise)
  - Modeling option 0: T ~ \alpha
  - Modeling option 1: T ~ \alpha.S (+ \beta) with S=M.N.K
  - Modeling option 2: T ~ \alpha.M.N.K + \beta.M.N + \gamma.N.P + ..
  - Modeling option 0',1',2': T ~ if(M<\Sigma_1..) {...} else {} where the
    \Sigma's have to be estimated.
  - Noise option 0: T ~ ... + 0
  - Noise option 1: T ~ ... + gausian noise
  - Noise option 2: T ~ ... + noise that depends on parameters
  - Noise option 3: T ~ ... + specific asymetric noise that depends on
    parameters
  - Noise option 0',1',2',3': mixture \pi_1 , \dots, \pi_k
- Injecting the model in the simulation is easy but how is done the
  estimation ?
  |    | N0                | N1                | N2                    | N3                                       |
  |----+-------------------+-------------------+-----------------------+------------------------------------------|
  ...
  - Tools:
    - mean, sd, histogram sampling, linear regression are standard
      tool in R, python/statsmodels
    - Bayesian sampling requires more elaborate tool like STAN or
      PyMC3. The probabilistic model is formally stated.
  - All M' options, require N0 or N1. Several implementations are
    available: Yet, N1 can generally be obtained by averaging
    sufficiently over input parameters.
    - M'0: cubist (piecewise constant, multi-dimensional),
    - M'1: segmented (piecewise linear but continuous) or our own
      implementation based on the Hastie, Tibshirani, Friedman book.
    - M'2: ok if model is given, more difficult if model has to be
      identified.
  - N' options generally require N0 or N1. 
    - M0 + N'1: clustering
    - M1 + N'1: flexmix
...
Full modeling of DGEMM (Modeling option 2 + Noise option 4) is
  - T ~ \alpha.M.N.K + \beta.M.N + ... +  |N(0,\alpha'.MNK + ....) |
  - T ~ \alpha (M.N.K + |N(0,\sigma^2)|) but part of the noise can be explained
    by M, N, and K.
  - T ~ \alpha.M.N.K
The consequence of using simpler options are already visible but will
be illustrated later.
** Modeling Minor kernels
Decent modeling of Minor kernels (Modeling option 1 + Noise option 4)
  - T ~ \alpha.(size + |N(0,\sigma)|)
The consequence of using simpler options will be illustrated later.
