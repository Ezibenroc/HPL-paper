...
- We used a smaller block size than what is usually done, this leads to an
...
charts of the simulations with the reality (see figure
\ref{fig:gantt_simulation}). In the charts, calls to \texttt{dgemm} are in
...
this metric in reality and in simulation in figure
\ref{fig:validation_performance}.
...
* Comparing with a real qualification run (Stampede)
