...
interesting to note that $M.N.K$ is not sufficient to fully describe
...
lab = lab %>% mutate(label = case_when(model == 'A1'     ~ 'simple',
                                       ...
                                       model == 'A1_het' ~ 'heterogeneous dgemm',
                                       model == 'C1_het' ~ 'heterogeneous dgemm\n& polynomial dgemm',
                                       model == 'D1_het' ~ 'heterogeneous dgemm\n& polynomial dgemm\n& stochastic kernels',
                                       model == 'D2_het' ~ 'heterogeneous dgemm\n& polynomial dgemm\n& stochastic kernels\n& stochastic network',
...
lab = lab %>% mutate(xl = case_when(model == 'A1'     ~ xl-delta_x,
                                    model == 'A2'     ~ xl+delta_x,
                                    model == 'A1_het' ~ xl-1.7*delta_x,
                                    model == 'C1_het' ~ xl+1.7*delta_x,
                                    model == 'D1_het' ~ xl-1.2*delta_x,
                                    model == 'D2_het' ~ xl+2.2*delta_x,
                                       T             ~ xl))
lab = lab %>% mutate(yl = case_when(model == 'A1'     ~ yl+delta_y,
                                    model == 'A2'     ~ yl+delta_y,
                                    model == 'A1_het' ~ yl+0.5*delta_y,
...
#+RESULTS:
[[file:/tmp/babel-fhdEeb/figure8QFZF1.png]]
...
network model, \noisep{1}, shown in yellow. This does not cause any significant
change in HPL performance prediction, as shown by the plot.
...
model (thus switching from \model[H]{1} to \model[H]{2}) has an higher impact on
the performance prediction for smaller matrices. This new model, depicted in
orange, is very close to the reality at the beginning, but the larger the
matrix, the closer it comes to the previous model.

Finally, adding some noise (\noise{2} for all kernels, \noise[H]{2} for
\texttt{dgemm}) helps a lot to get the last bit of realism. The prediction, in
blue, is now extremly close to the reality as it underestimates the performance
by less than 5%, even 2% for the larger matrices. Adding back some noise to the
network model (\noisep{1}, in pink) is still not significant.
...
individual resource is realized.
