...
such that $A=L\times U$ holds true. The sequential complexity of such
operation is given by $\mathrm{flop}(N) = \frac{2}{3}N^3 + 2N^2 +
O(N)$.
...
\begin{tikzpicture}[scale=0.32]
    ...
    \node(L) at (2, 2) {\ensuremath{\mathbf{L}}};
    \node(U) at (10, 10) {\ensuremath{\mathbf{U}}};
    \node(A) at (8, 4) {\ensuremath{\mathbf{A}}};
...
** TODO A typical run on a supercomputer
:LOGBOOK:
- State "TODO"       from              [2017-11-15 mer. 16:26]
:END:
\AL{Describe stampede (already done in Chapter VII (=modeling Stampede= and =Network Topology=)) and the stampede fullrun.}
...
\begin{figure}%[!htb]
  ...
  \end{minipage}}
  \caption{HPL output submitted in November 2013 for the ranking of stampede in the top500.}
\end{figure}
#+END_EXPORT
...
\AL{Too detailed on simulation. Difficulties = difficulties in term of performance prediction, i.e., application, runtime and platform and no formula may account for such complexity. These difficulties are rather conclusions of the related work section.}
   Several difficulties were well-known in advance and had to be resolved in
   ...
   1. The time-complexity of the algorithm is $\O(N^3)$ and
      ...
      become rather slow. For instance, the real-life run on the Stampede cluster took almost
