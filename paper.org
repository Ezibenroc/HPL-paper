...
\modelp[H]{1} for the network with no noise (\noise{0}) in both cases. This model
...
prediction that is consistent with the performance obtained a few
months ago.

We have thus shown it is possible to consistently predict the
