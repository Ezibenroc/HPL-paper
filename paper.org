...
framework that allow us to emulate HPL at the scale of a
...
that rely on =MPI_Iprobe= to facilitate overlapping with computations.
...
The Stampede run of HPL was done several years ago and we only
...
machine was impossible for us. We modeled Stampede carefully, which
allowed us to predict the performance that would
have been obtained using the unmodified, freely available version of HPL.
...
misreported in the output. An analysis of the optimized
