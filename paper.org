...
The largest simulation took, with all previously described
optimizations enabled, close to $47$ hours and consumed
...
process number or matrix order on total makespan and memory. 
When the matrix order ($N$) is increased, memory consumption and
...
\CH{There are no lower panels in that figure anymore. Remove?}

For all these simulations, the CPU utilization is above \SI{98}{\percent}. This
means that the kernel is still able to manage the page table without
stalling too much the simulation process.
#+LATEX: \CH{We already reported this before. Remove?}
...
As these experiments worked out satisfyingly, we can now try to
simulate with Stampede one of the largest supercomputers.
...
context and SMPI was successfully validated in several small-scale
...
supercomputer. HPL is particularly challenging in terms of simulation
as it implements its own set of non-blocking collective operations
relying on =MPI_Iprobe= that are *intertwined* with computations.
...
Our emulation of a similar configuration runs on a single machine for
about $47$ hours and requires less than \SI{16}{\giga\byte} of RAM. This emulation
...
The downside of this large scale is a less well-controlled scenario
...
machine was impossible for us. We conduct a careful modeling of Stampede, which
allows us to obtain the performance that would
...
was much lower than what was reported in 2013. This
discrepancy can be explained *by the fact* that a closed-source version
specifically designed and optimized by the Intel engineers was used in 2013.
We believe that through these changes, some of the HPL configuration parameters were
misreported in the output. Our analysis of the optimized
HPL binary confirmed that algorithmic differences are the
reason for the performance differences.

We conclude that our large-scale (in)validation is unfortunately not
possible as we do not have access to the source code with the changes
made by the Intel engineers. We claim that the modifications we did are
minor and should be applicable to their version. Indeed, while HPL
