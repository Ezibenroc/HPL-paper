...
=panel=. Indeed, =panel= is a mixed data structure with both \texttt{int}s,
accounting for matrix indices, error codes, and pivoting information,
...
allocation of \texttt{double}s. Using a fully shared memory allocation
