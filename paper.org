...
\texttt{Rmax}      & \NSI{5168.1}{\tera\flops} & 5,884.6 & 24.55              \\
...
iteration of HPL typically weights a few hundreds of MB.
...
Emulation of HPL comes with at least two challenges:
...
- Secondly, the tremendous memory consumption and 
  number of memory accesses need to be dealt with.
...
C/C++ or FORTRAN\cite{smpi}.  main development goal for SimGrid has
...
to confirm or debunk network and application models. Recent
results report consistent predictions within a
...
- *Memory folding*: SMPI provides the =SMPI_SHARED_MALLOC= (=SMPI_SHARED_FREE=) macro to
     ...
     even be overwritten. 
     =SMPI_SHARED_MALLOC= works as follows (see Figure\ref{fig:global_shared_malloc}) : a single block of physical memory (of default size \NSI{1}{\mega\byte}) for the whole
...
allocation should be preserved (i.e., private to each process) and
which ones may be corrupted (i.e., shared between process). 
...
\Num{6880} (with =dlopen=). This results in a reduction of system time from 
...
\NSI{50}{\giga\byte\per\sec} and a latency of \NSI{5}{\micro\sec}. We ran
simulations with $512$; $1,024$; $2,048$ or $4,096$ MPI processes and
with matrices of orders \Num{5e5}, \Num{1e6}, \Num{2e6} or \Num{4e6}.
...
\NSI{16}{\giga\byte} of memory whereas the smallest one took $20$ minutes and \NSI{282}{\mega\byte} of memory.
...
Moreover, all the simulations spend less than \NSI{10}{\percent} of their execution time in kernel
