...
#+LaTeX: \AL[]{We should talk about dev and vendors at once}
...
emulate detailed scenarios similar to the HPL run conducted on the
Stampede cluster (TACC) in 2013 for the TOP500 on a single commodity
sever and in less than two days. More importantly, we manage to
predict the performance of HPL on a controlled cluster (with a thousand of
...
the High-Performance Linpack benchmark\cite{HPL}, HPL, as a research
vehicle. HPL is used to benchmark systems for the TOP500\cite{TOP500}
list and implements a LU decomposition using MPI. 
HPL checks the correctness of this factorization by
...
*** Kernel Modeling
Given its complexity, a faithful prediction of HPL requires the
emulation of its code.  HPL heavily relies 
on BLAS kernels such as =dgemm= (for matrix-matrix multiplication) or
=dtrsm= (for solving an equation of the form $A\cdot x=b$). Analyzing an
HPL simulation with $64$ processes and a very small matrix of order
$30,000$ shows that roughly \NSI{96}{\percent} of the time is spent in
...
estimated time for the real kernel is calculated based on the given
...
In addition to the main computation kernels, 
...
allows to load several times the =data= segment (which contains global
variables) in memory and to avoid costly
calls (and cache flushing) to =mmap= when context switching. For a
...
fat-tree topology.
We run simulations with $512$; $1,024$; $2,048$ or
$4,096$ MPI ranks and with matrices of orders \Num{5e5},
...
- Noise option \noise{2} (heteroscedastic): The conditional variance
  of the residuals (\ie $\sigma^2$ given $M,N,K$) is modeled by polynomial
...
\begin{figure}[!t]
  \vspace{-1em}
  \centering \includegraphics[width=\linewidth]{./figures/network/calibration.png}
  \vspace{-2em}
...
\begin{figure*}[!b]
  ...
  \begin{minipage}[t]{\linewidth}\null\vspace{-.75em}
    \centering
    \subfigure[\texttt{dgemm} heterogeneity\label{fig:dgemm_het}]{\includegraphics[width=.33\linewidth]{figures/kernels/dgemm_heterogeneity_calib.png}}%
    \subfigure[\texttt{dgemm} model\label{fig:dgemm_poly}]{\includegraphics[width=.33\linewidth]{figures/kernels/dgemm_model_calib.png}}%
...
distinguishable from the real behavior. For all these kernels, similar
results can be obtained with this category of model.
** Clearpage                                               :noexport:ignore:
...
\begin{figure}[h]
    \centering
    \resizebox{0.95\linewidth}{!}{\input{figures/validation_performance.tex}}\vspace{-1em}
    % \includegraphics[width=0.95\linewidth]{./figures/validation_performance.pdf}
    \caption{HPL performance: predictions vs. reality}\vspace{-2em}
...
%  \includegraphics[width=\linewidth,page=1]{./figures/stampede_simgrid.pdf}
  ...
  \caption{HPL on Stampede: predictions vs. reality}\vspace{-2em}
...
about $62$ hours and required less than \NSI{19}{\giga\byte} of RAM. 
#+LaTeX: \AL[]{Inconsistent values}
This emulation
...
We explain how to model the performance of computation kernels and of
...
years ago and we only have very limited information about the setup
(\eg software versions and configuration), but a reservation and
...
freely available version of HPL.  Unfortunately, despite all our
...
differences.  We claim that the modifications we made are minor and
...
As a future work,
we intend to automate our modeling approach as much as possible and to
conduct similar studies with other HPC benchmarks (\eg HPCG or HPGMG)
and with other TOP500 machines. In this context, we believe that a
faithful and public reporting of the experimental conditions (compiler
options, library versions, HPL output, etc.) is invaluable and allows
...
run of Theta.
** References                                                       :ignore:
\AL[]{Ajouter des refs pour nous cacher un peu. ;)}
