...
=dtrsm= (for solving an  $A\cdot x=b$ equation). Analyzing an
HPL simulation with $64$ processes and a very small matrix of order
$30,000$ shows that $\approx \NSI{96}{\percent}$ of the time is spent in
...
correctness verification, is not considered in the reported
performance.  Since both phases have no impact on the reported
performance, we can safely skip them.
Last, note that HPL
implements an LU factorization with partial pivoting, which requires a special
...
which ones may be corrupted (\ie shared between processes). 
For a matrix of order $40,000$ and $64$ MPI processes,
memory consumption decreases from about \NSI{13.5}{\giga\byte} to
