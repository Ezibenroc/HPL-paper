...
\title{Fast and Faithful Performance Prediction of\\ MPI applications: the HPL use case study}
...
\begin{center}
    ...
    \begin{minipage}[t]{0.48\linewidth}
        \vspace{0pt}
        \includegraphics[width=\linewidth]{figures/validation_performance_simple.pdf}
        \label{fig:teaser:performance}
        \vspace{-0.5cm}
        \captionof{figure}{Dahu's predicted performance is within a few percents of reality, using a stochastic heterogeneous model for \texttt{dgemm} and a simplistic model for the other kernels and the nerwork}
...
# Plot drawing
plot = ggplot(df, aes(x=matrix_size, y=gflops, color=origin, linetype=origin))
plot = plot + stat_summary(fun.y='mean', geom='line')
plot = plot + stat_summary(fun.y='mean', geom='point')
plot = plot + theme_bw() + expand_limits(y=0)
plot = plot + scale_color_brewer(type='qual', palette='Dark2', guide=F)
plot = plot + scale_linetype_manual(values=c('solid', 'dashed'), guide=F)
plot = plot + theme(text=element_text(size=12))
plot = plot + ylab('Gflop/s') + xlab('Matrix rank')
plot = plot + geom_text(data=lab, aes(x=x, y=y, label=origin), size=4)
plot = plot + theme(text=element_text(size=text_size))
ggsave(filename='figures/validation_performance_simple.pdf', height=3, width=6, plot=plot)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-fPmClk/figurexDVPFa.png]]
