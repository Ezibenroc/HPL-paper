...
\begin{figure*}%[!htb]
  ...
  \begin{minipage}[b]{.9\linewidth}
    \lstset{basicstyle=\ttfamily, frame=bt,backgroundcolor=\color{white},numbers=none,numberstyle=\tt\prettysmall,escapechar=|}\lstinputlisting{HPL_dtrsm_macro.c}%
  \end{minipage}
  \caption{Replacing the calls to computationnaly expensive functions by a model.\label{fig:choleskycode}}
\end{figure*}
#+END_EXPORT

#+BEGIN_SRC C :exports none :tangle HPL_dtrsm_macro.c
