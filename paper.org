...
#+BEGIN_EXPORT latex
\begin{figure}
  \centering \subfigure[Stampede]{\includegraphics[width=.45\linewidth,page=1]{./figures/network/stampede.png}}
  \subfigure[Dahu]{\includegraphics[width=.45\linewidth,page=1]{./figures/network/dahu.png}}
...
%  \begin{minipage}{.5\linewidth}
    \subfigure[\texttt{dgemm} heterogeneity]{\includegraphics[width=.8\linewidth]{figures/kernels/dgemm_heterogeneity.pdf}}

    \subfigure[\texttt{dgemm} model]{\includegraphics[width=.8\linewidth]{figures/kernels/dgemm_model.png}}
...
on Figure\ref{fig:nw_var}\AL[]{$\to$ Tom: Regenerate}, the variability of highe speed networks can
...
draw_reg <- function(df, breakpoints) {
    ...
    plot = ggplot(df, aes(x=msg_size, y=duration, color=group)) + geom_point(alpha=0.1)
    plot = plot + scale_x_log10(breaks=c(1e1, 1e3, 1e5)) + scale_y_log10() + theme_bw() + scale_color_discrete(guide=F)
    func = unique(df$func)
    plot = plot + xlab('Message size (B)') + ylab('Duration (s)')
    plot = plot + expand_limits(x=980284, y=0.00022)  # to get the same scale on all plots
    plot = plot + expand_limits(x=1, y=1e-07)
    plot = plot + theme(text = element_text(size=font_size))
    plot = plot + annotate(geom='text', label=func, size=font_size%/%3, x=10, y=5e-5)
...
draw_mpi_reg <- function(platform, breakpoints) {
    ...
    # Dahu's calibration has 10 times more points than Stampede, so let's remove some points to get similar plots
    df_send = df_send[sample(nrow(df_send), 33100), ]
    df_recv = df_recv[sample(nrow(df_recv), 33100), ]
...
ggsave(filename='figures/network/dahu.png', plot=plot)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-I9Qhts/figuredzU4eP.png]]

#+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 600 :session *R*
plot = draw_mpi_reg('stampede', c(150, 5000, 17420, 110000))
ggsave(filename='figures/network/stampede.png', plot=plot)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-I9Qhts/figureilT40P.png]]
...
df$mk = df$m * df$k
df$nk = df$n * df$k
df = df[df$mnk < 2e10,]  # removing the few very large calls, to not pollute the plot
head(df)
#+end_src
...
6 dgemm 9441  640 1160  3475.796 0.4544535  0.4845474  0.0007154680  0.4852629
  node core cpu index index_in_seq        mnk      mn       mk       nk
1   10    0  20     0            0 7008981840 2887920   917406 18542280
2   10    0  20     1            1 7008981840 2887920   917406 18542280
3   10    0  20     2            2 7008981840 2887920   917406 18542280
4   10    0  20     3            0 7008998400 6042240 10951560   742400
5   10    0  20     4            1 7008998400 6042240 10951560   742400
6   10    0  20     5            2 7008998400 6042240 10951560   742400
#+end_example

Regression lines, to show the heterogeneity.
#+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session *R*
plot = ggplot(df, aes(x=mnk, y=duration, color=factor(cpu))) + geom_smooth(method='lm')
plot = plot + theme_bw() + ylab('Duration (s)') + xlab('M×N×K') + labs(color='CPU')
plot = plot + scale_color_discrete(guide=F)
plot = plot + theme(text = element_text(size=30))
plot = plot + scale_x_continuous(breaks=c(0, 3e9, 6e9, 9e9))
ggsave(filename="figures/kernels/dgemm_heterogeneity.pdf", plot=plot)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-UOcW8p/figureqBMG2s.png]]

Scatter plot for one CPU, to show the time variability. Our prediction here is
deterministic, the "variability" we can observe is entirely due to the
polynomial model, no noise is injected.
#+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session *R*
tmp = df[(df$cpu == 4),]
plot = ggplot(tmp, aes(x=m*n*k, y=duration)) + geom_point(alpha=0.3) + geom_point(aes(y=prediction), alpha=0.3, color='red')
plot = plot + theme_bw()
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-UOcW8p/figurekEhoHL.png]]

Scatter plot to show our model.
#+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 800 :height 400 :session *R*
library(reshape2)
tmp = df[sample(nrow(df), 100000), ]
names(tmp)[names(tmp) == 'duration'] <- 'reality'
names(tmp)[names(tmp) == 'pred_noise'] <- 'simulation'
tmp = melt(tmp, id.vars=c('mnk', 'cpu'), measure.vars=c('reality', 'prediction'))
plot = ggplot(tmp, aes(x=mnk, y=value, color=factor(cpu))) + geom_point(alpha=0.3)
plot = plot + theme_bw() + ylab('Duration (s)') + xlab('M×N×K')
plot = plot + scale_color_discrete(guide=F)
plot = plot + theme(text = element_text(size=30))
plot = plot + scale_x_continuous(breaks=c(2.5e9, 7.5e9))
...
#+RESULTS:
[[file:/tmp/babel-UOcW8p/figureUcrYc6.png]]
...
plot = ggplot(func, aes(x=m*n, y=duration, color=mode)) + geom_point()
plot = plot + theme_bw() + ylab('Duration (s)') + xlab('M×N')
plot = plot + theme(text = element_text(size=30))
plot = plot + scale_x_continuous(breaks=c(0, 1e6))
plot = plot + theme(legend.position = c(0.2, 0.9)) + theme(legend.title=element_blank())
ggsave(filename="figures/kernels/dlatcpy_model.png", plot=plot)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-UOcW8p/figure1CSno6.png]]
...
repository, no need to download them. They have been dumped by [[https://github.com/Ezibenroc/mpi_calibration/blob/b4905fc175ffd5f0431af2595541cd379d2ed50b/dahu/smpi_hpl/hpl_simulation.ipynb][this notebook]]
(cell 2).
...
- Simulation with the heterogeneous dgemm model: [[https://github.com/Ezibenroc/mpi_calibration/tree/master/dahu/smpi_hpl/paper_sc19/performance/10][these archives]].
*** Drawing the plot
#+begin_src R :results output :session *R* :exports both
library(ggplot2)
library(dplyr)
...
1      125000  18260   71.31 simulation           True               True
2      250000  20790  501.01 simulation           True               True
3      450000  21600 2812.52 simulation           True               True
4      100000  17700   37.67 simulation           True               True
5      200000  20130  264.91 simulation           True               True
6      400000  21390 1994.66 simulation           True               True
...
text_size = 6
# Labels definitions
lab = dfsim %>% filter(matrix_size == 300000) %>% select(gflops, matrix_size, model) %>% distinct()
lab = lab %>% mutate(label = case_when(model == 'A1'     ~ 'simple',
                                       model == 'A2'     ~ 'stochastic network',
                                       model == 'A1_het' ~ 'heterogeneous',
                                       model == 'C1_het' ~ 'heterogeneous\n&polynomial dgemm',
                                       model == 'D1_het' ~ 'heterogeneous\n&polynomial dgemm\n&stochastic kernels',
                                       model == 'D2_het' ~ 'heterogeneous\n&polynomial dgemm\n&stochastic kernels\n&stochastic network',
                                       T             ~ 'undefined'))
lab$x = lab$matrix_size
lab$y = lab$gflops
lab$xl = lab$x*1.0
lab$yl = lab$y*1.0
delta_x = max(dfsim$matrix_size) / 15
delta_y = max(dfsim$gflops) / 15
lab = lab %>% mutate(xl = case_when(model == 'A1'     ~ xl-delta_x,
                                    model == 'A2'     ~ xl+delta_x,
                                    model == 'A1_het' ~ xl-1.5*delta_x,
                                    model == 'C1_het' ~ xl+1.5*delta_x,
                                    model == 'D1_het' ~ xl-delta_x,
                                    model == 'D2_het' ~ xl+2*delta_x,
                                       T             ~ xl))
lab = lab %>% mutate(yl = case_when(model == 'A1'     ~ yl+delta_y,
                                    model == 'A2'     ~ yl+delta_y,
                                    model == 'A1_het' ~ yl+delta_y,
                                    model == 'C1_het' ~ yl+1.2*delta_y,
                                    model == 'D1_het' ~ yl-2*delta_y,
                                    model == 'D2_het' ~ yl-2*delta_y,
                                       T             ~ yl))

# Plot drawing
plot = ggplot(dfsim, aes(x=matrix_size, y=gflops)) + geom_point(data=dfreal, shape=1) + geom_point(aes(color=model))
plot = plot + stat_summary(aes(color=model), fun_y=mean, geom='line', linetype='dashed')
plot = plot + stat_summary(fun_y=mean, geom='line', data=dfreal)
...
plot = plot + geom_segment(data=lab, x=4.7e5, xend=4.5e5, y=2.5e4, yend=mean((dfreal %>% filter(matrix_size == 450000))$gflops), linetype="solid")
plot = plot + geom_label(data=lab, aes(x=4.7e5, y=2.5e4), label='reality', size=text_size)
ggsave(filename='figures/validation_performance.pdf', plot=plot)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-xBbVhd/figurefowhPt.png]]
