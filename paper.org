...
=DEPTH=, \ldots). 
\AL{This last term is very difficult to estimate}
...
#+LaTeX:  \AL{Message: only scarce information is available from the TOP500}
In June 2013, the Stampede supercomputer at TACC was ranked 6th in the TOP500 by
achieving \SI{5168.1}{\tera\flops}. In November 2017, a performance of
\SI{5884.6}{\tera\flops} gave the 18th place to the Theta supercomputer at
ANL. Finally, we have personally performed a run of HPL on a Grid5000 cluster
named Dahu that reached a performance of \SI{24.55}{\tera\flops}. We report in
figure\ref{fig:typical_run} the parameters used for each of these runs.
...
\hline
% Rpeak & 8,520.1              & 9,627.2              & 62.26              \\
Rmax      & \SI{5168.1}{\tera\flops} & \SI{5884.6}{\tera\flops} & \SI{24.55}{\tera\flops}              \\
...
DEPTH     & 0                        & 0                        & 1                  \\
Time      & 2 hours                  & 28 hours                 & 1 hour             \\
Memory    & \SI{120}{\tera\byte}     & \SI{559}{\tera\byte}     & \SI{2}{\tera\byte} \\
...
The performance typically achieved by supercomputers (e.g.,
\SI{5168}{\tera\flops} for Stampede), needs to
be compared to the peak performance (\ie
\SI{8084}{\tera\flops} with 6,006 nodes for Stampede). This difference may be attributed to the node
...
#+BEGIN_EXPORT latex
\AL{HPL may be a benchmark, it presents very complex collective communication patterns that are entangled with the computing logic of the application to offer the best possible overlap of communications with computations. Its adaptive behavior is thus particularly challenging to simulate and representative of what can be found in many MPI applications.}
#+END_EXPORT
...
case for HPL.
\AL{Add info about trace size for 1 iteration}
...
the developments we propose would a priori also be possible with SST Macro. Emulation of
...
To this end, SMPI, a simulator based on SimGrid, has been developed
and used to faithfully simulate unmodified MPI applications written in
C/C++ or FORTRAN\cite{smpi}.  A main development goal for SimGrid has
been to provide validated performance models particularly for
scenarios leveraging the network.  Such a validation normally consists
of comparing simulation predictions with results from real experiments
to confirm or debunk network and application models.
In\cite{heinrich:hal-01523608}, a validation of SimGrid's energy
module was provided by accurately and consistently predicting within a
few percent the performance and the energy consumption of HPL and some
other benchmarks on small-scale clusters (up to $12\times12$ cores
in\cite{heinrich:hal-01523608} and up to $128\times1$ cores
in\cite{smpi}). In this article, validate this approach at a much
larger scale.
...
own synchronization semantics, which results in different effective
network's latency and bandwidth. 
...
This time can be scaled up or down depending on the simulated machine
with respect to the simulation machine.
...
function that allows to project several times the global variables in the =data= segment and to significantly save calls to =mmap= when context switching. The main advantage of this second approach is that caches do not need to be flushed as is the case for the =mmap= approach, because data consistency can always be guaranteed.
...
- *Memory folding*: SMPI provides the =SMPI_SHARED_MALLOC= (=SMPI_SHARED_FREE=) macro to
     ...
     physical address.
     This mechanism allows most applications to obtain a nearly constant memory
...
#+LaTeX: notebook\footnote{See \texttt{journal.org} at \url{https://github.com/Ezibenroc/simulating_mpi_applications_at_scale/}\AL{Anonymize}}.
...
$30,000$ showed that roughly \SI{96}{\percent} of the time is spent in
...
performance model for the respective kernel. Unfortunately despite 
this regularity, SimGrid's kernel sampling mechanism is completely ineffective
with HPL because the geometry the matrices passed to these kernels 
evolves in a very specific way during the execution. Furthermore, the 
performance is very dependent on the target machine BLAS library and 
would hardly be inferred from an online estimation on the simulation machine.
...
#+LaTeX: \AL{Mention extra work for \texttt{DEPTH}?}
...
** MPI memory virtualization
#+LaTeX: \AL{This needs to be shrinked and probably merged with the Huge pages section}
...
\TC{TODO: these plots have changed, scalability is less good}
\AL{Clarify that we use a configuration similar to the Stampede but that algorithm parameters (bcast, depth,...) obviously influence simulation time as they may induce more complex behaviors.}
...
In the following, we denote as $T$ the duration of an operation whose
parameters are $M$, $N$, $K$, \dots In the case of the =dgemm=
(matrix-matrix product) operation, these parameters would describe the
geometry of the input matrices. The first three modeling options we
consider are the following:
- Modeling option \model{0}: The duration is modeled as a constant
  duration independent of the input parameters, i.e. $T \sim \alpha$, where
  $\alpha$ is estimated through the sample average of the duration of the
  operation. This is a reasonable model for simple and stable
  computation kernels.
- Modeling option \model{1}: a simple combination of the parameters
  (e.g., $S=M.N.K$) is the primary factor driving the performance of
  the operation, i.e. $T \sim \alpha.S ~(+ \beta)$. $\alpha$ and $\beta$ are then simply
  estimated through a classical least-square linear regression.
- Modeling option \model{2}: the behavior of the operation is complex
  or requires a faithful modeling over the full range of input
  parameters and a full polynomial model is then required, i.e. $T \sim
  \alpha.M.N.K + \beta.M.N + \gamma.N.P + \dots$. Again, the $\alpha,\beta,\gamma,\dots$ can then be
  simply estimated through a classical least-square linear regression.
...
- The platform may be slightly heterogeneous (spatial variability) and
  ...
  individually. We denote this modeling option as \model[H]{} but
...
- The behavior of the operation may be mosly linear but solely on
  ...
  networking operations or for computing nodes on Stamped where the
  ...
  $T \sim \begin{cases} 
      ...
      ... \end{cases},$
  #+END_EXPORT
  where the $\theta, \alpha, \beta$ should be all be experimentally to be estimated.
  We denote this kind of model as \modelp{}.

All previous models can be easily fit with relatively simple linear
regressions or maximum likelihood learning methods. However, an
important hypothesis underlying all these methods is that the model is
homoscedastic, i.e., that the variability is fully independent on the
parameters. The residual (temporal) variability may an important
phenomenon to account for as "system noise" is knows to be detrimental
to overall performance of parallel applications like HPL. We therefore
consider different modeling options for this temporal variability.
- Noise option \noise{0}: the simplest option consists in simply
  injecting the value predicted by the model, i.e., with no noise at
  all $T \sim \model{}(M,N,K) + 0$.
- Noise option \noise{1}: The simplest probability family to model
  variability is the normal distribution, hence $T \sim \model{}(M,N,K) +
  \norm(0,\sigma^2)$, where $\sigma^2$ can easily be estimated through the sample
  variance of the model residuals.
- Noise option \noise{2}: The residuals are modeled as a normal
  distribution whose variance is a polynomial function of the input
  parameters.
# - Noise option 3: T \sim ... + specific asymetric noise that depends on
#   parameters
...
* Comparing with the Stamped qualification run
