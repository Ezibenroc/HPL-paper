...
\author{
    ...
    \IEEEauthorblockA{Univ. Grenoble Alpes, CNRS, Inria, Grenoble INP, LIG\\ 38000 Grenoble, France\\
      ...
      \includegraphics[width=.75\linewidth]{figures/validation_performance_simple.pdf}
    }
}
...
Usual MPI applications allow their users to choose between several
parameters. For instance, it is often possible to select the virtual topology,
the collective operation implementation and the granularity level. This tuning
is critical to benefit from the highest performance.  With a rising cost of
modern supercomputers, running parallel applications at scale solely to optimize
their performance is extremely expensive. Having cheap but faithful predictions
of said performance could be a great help for researchers and system
administrators.

In this article, we explain how we managed to perform faithful simulations of
High Performance Linpack (HPL), the benchmark used to rank supercomputers in the
TOP500. We both extended the SimGrid's SMPI simulator and slightly modified the
open-source version of HPL to allow a fast emulation on a single commodity
server at the scale of a supercomputer. We explain how to model the different
components (network, BLAS, ...) and show that a careful modeling of both spatial
and temporal node variability allows us to obtain predictions within a few
percents of real experiments.
#+LaTeX: \end{abstract}
* Introduction
...
repository, no need to download them. They have been dumped by [[https://github.com/Ezibenroc/mpi_calibration/blob/230cd9f1a9a985eecd2aea5098c2a8b643c16d66/dahu/smpi_hpl/hpl_simulation.ipynb][this notebook]]
(cell 2).
...
max_x = max(dfsim$matrix_size)
max_y = max(dfsim$gflops)
lab = lab %>% mutate(x  = case_when(origin == 'Simulation' ~ 0.95*x,
                                    origin == 'Reality'    ~ 1.05*x
                                   ))
...
plot = plot + theme(text=element_text(size=12))
plot = plot + ylab('Gflop/s') + xlab('Matrix rank')
plot = plot + geom_text(data=lab, aes(x=x, y=y, label=origin), size=6)
plot = plot + theme(text=element_text(size=text_size))
ggsave(filename='figures/validation_performance_simple.pdf', height=3, width=9, plot=plot)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-nl8reK/figure76mhSq.png]]
