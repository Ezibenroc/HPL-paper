...
With a rising power consumption on modern supercomputers, running parallel
applications at scale solely to optimize their performance is extremely
expensive. However, this tuning is critical to benefit from the highest
performance, as one often has to choose between different settings, like the
granularity level or the collective operation implementation.
